...
if [ "$global" == "" ]
then
    echo "${files[*]}" | dotnet trwdog -- java -classpath "$CLASSPATH" it.univr.injectiontool.javascript.Test -q -x -tree > parse.txt 2>&1
else
    echo "${files[*]}" | trwdog java -classpath "$CLASSPATH" it.univr.injectiontool.javascript.Test -q -x -tree > parse.txt 2>&1
fi
status=$?
